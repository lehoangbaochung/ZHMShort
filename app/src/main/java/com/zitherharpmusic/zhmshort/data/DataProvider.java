...
public class DataProvider {
    ...
    private static String getString(JSONArray jsonArray, int index) throws JSONException {
        return !jsonArray.isNull(index) ? jsonArray.getString(index) : Music.EMPTY_CHARACTER;
    }
    ...
    private static void loadSongs() throws JSONException {
        JSONArray jsonArray = getJSONArray(getSheetUrl("audio"), "values");
        for (int i = 0; i < jsonArray.length(); i++) {
            valueArray = jsonArray.getJSONArray(i);
            primaryId = getString(valueArray, Music.PRIMARY_ID);
            referenceId = getString(valueArray, Music.REFERENCE_ID);
            vietnameseName = getString(valueArray, Music.VIETNAMESE_NAME);
            pinyinName = getString(valueArray, Music.PINYIN_NAME);
            simplifiedChineseName = getString(valueArray, Music.SIMPLIFIED_CHINESE_NAME);
            traditionalChineseName = getString(valueArray, Music.TRADITIONAL_CHINESE_NAME);
            vietnameseDescription = getString(valueArray, Music.VIETNAMESE_DESCRIPTION);
            simplifiedChineseDescription = getString(valueArray, Music.SIMPLIFIED_CHINESE_DESCRIPTION);
            traditionalChineseDescription = getString(valueArray, Music.TRADITIONAL_CHINESE_DESCRIPTION);
            if (!primaryId.equals(Music.EMPTY_CHARACTER)) {
    ...
    private static void loadVideos() throws JSONException {
        JSONArray jsonArray = getJSONArray(getSheetUrl("short"), "values");
        String artistId, songId;
        for (int i = 0; i < jsonArray.length(); i++) {
            valueArray = jsonArray.getJSONArray(i);
            primaryId = getString(valueArray, Music.PRIMARY_ID);
            artistId = getString(valueArray, Video.ARTIST_ID);
            songId = getString(valueArray, Video.SONG_ID);
    ...
    private static void loadArtists() throws JSONException {
        JSONArray jsonArray = getJSONArray(getSheetUrl("artist"), "values");
        for (int i = 0; i < jsonArray.length(); i++) {
            valueArray = jsonArray.getJSONArray(i);
            primaryId = getString(valueArray, Music.PRIMARY_ID);
            vietnameseName = getString(valueArray, Music.VIETNAMESE_NAME);
